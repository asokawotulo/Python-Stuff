...
def dydx(state, time):
	...
	division_one = length_one*(mass_sum) - mass_two*length_one*np.cos(angle_diff)**2
	division_two = length_two*(mass_sum) - mass_two*length_two*np.cos(angle_diff)**2

	# Derivative of angular velocity
	omega_one_der = (-mass_two*length_one*angle_one_der**2*np.sin(angle_diff)*np.cos(angle_diff) + gravity*mass_two*np.sin(angle_two)*np.cos(angle_diff) - mass_two*length_two*angle_two_der**2*np.sin(angle_diff) - (mass_sum)*gravity*np.sin(angle_one))/division_one
	omega_two_der = (mass_two*length_two*angle_two_der**2*np.sin(angle_diff)*np.cos(angle_diff) + gravity*np.sin(angle_one)*np.cos(angle_diff)*(mass_sum) + length_one*angle_one_der**2*np.sin(angle_diff)*(mass_sum) - gravity*np.sin(angle_two)*(mass_sum))/division_two
...
incre_time = 0.001 # Increment Time (s)
array_time = np.arange(start_time, total_time, incre_time)
...
ax.text(0.01, max_space, "D O U B L E   P E N D U L U M", ha="left", color="#000000", fontsize=12)

# Time Template
time_template = 'Time = %.2fs'
time_text = ax.text(0.01, 0.925, '', transform=ax.transAxes, color="#000000")
...
x_one =  length_one*np.sin(integration[:, 0])
x_two =  length_two*np.sin(integration[:, 2]) + x_one
y_one = -length_one*np.cos(integration[:, 0])
y_two = -length_two*np.cos(integration[:, 2]) + y_one
...
# Animate Function
ani = animation.FuncAnimation(fig, animate, np.arange(1, len(integration)), interval=1, init_func=init, repeat=False)
